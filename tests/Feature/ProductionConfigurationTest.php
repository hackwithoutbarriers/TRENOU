...
class ProductionConfigurationTest extends TestCase
{
    ...
    public function test_authenticated_admin_dashboard_exposes_contact_messages_navigation(): void
    {
        $admin = User::factory()->create([
            'status' => User::STATUS_APPROVED,
            'is_superuser' => true,
        ...
        $this->actingAs($admin)
            ->get('/admin')
            ->assertOk()
            ->assertSee('Messages de contact')
            ->assertSee('/admin/contact-messages', false);
    }
}

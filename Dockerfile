...
CMD ["sh", "-c", "php artisan storage:link --force && php artisan migrate --force && php artisan db:seed --class=ProjetSeeder --force && php artisan config:cache && php artisan app:provision-admin && php artisan route:cache && php artisan view:cache && apache2-foreground"]
